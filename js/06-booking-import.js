(function(){
  ...
  const VYVA_BOOKING_IMPORT_VERSION='vyva-booking-autoimport-v10-route-names';
  ...
  function flightCandidates(text){
    ...
    const push=(raw,index,score,labelled=false)=>{
      ...
      if(!value)return;
      // Short OCR noise such as "AA1" is common in image decorations and
      // must not split the real booking into a false extra flight block.
      const digits=(value.match(/\d+/)||[''])[0];
      if(!labelled&&digits.length<2)return;
  ...
  function bookingCodeCandidates(text){
    const u=upperText(text),out=[];
    const labels=[
      /(?:NUMERO\s+DI\s+RIFERIMENTO(?:\s+DELLA)?\s+PRENOTAZIONE|RIFERIMENTO\s+(?:DELLA\s+)?PRENOTAZIONE)\s*[:#-]?\s*([A-Z0-9-]{4,12})\b/g,
  ...
  // OCR often misreads three-letter airport codes in large decorative fonts.
  // City/airport-name words are safer route anchors in that case.
  function airportRouteTokens(a){
    const ignored=new Set(['AIRPORT','AEROPORTO','INTERNATIONAL','INTERNAZIONALE','LONDON','MILAN']);
    const tokens=[];
    for(const raw of airportSearchTokens(a)){
      tokens.push(raw);
      for(const part of raw.split(/[^A-Z0-9]+/)){
        if(part.length>=4&&!ignored.has(part))tokens.push(part);
      }
    }
    return uniq(tokens.filter(x=>x.length>=3));
  }

  function airportAnchorIndex(text,airport){
    const u=upperText(text);
    let best=-1;
    for(const token of airportRouteTokens(airport)){
      const i=u.search(new RegExp('\\b'+token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'\\b'));
  ...
  function safeRouteTerminals(text,fromAirport,toAirport){
    ...
    const route=rows.findIndex(r=>{
      const a=airportAnchorIndex(r,fromAirport),b=airportAnchorIndex(r,toAirport);
      return a>=0&&b>a;
    });
    if(route>=0){
    ...
    const anchors=rows.map((r,i)=>({
      i,
      from:airportAnchorIndex(r,fromAirport)>=0,
      to:airportAnchorIndex(r,toAirport)>=0
    }));
  ...
  function selectLegWindow(text,t,leg){
    ...
    const routeOK=block=>{
      ...
      const a=airportAnchorIndex(block,e.from);
      const b=airportAnchorIndex(block,e.to);
      return a>=0&&b>a;
    };
    ...
    if(blocks.length===1&&expected===blocks[0].f.value){
      const fromPos=airportAnchorIndex(u,e.from),toPos=airportAnchorIndex(u,e.to);
      const reverse=fromPos>=0&&toPos>=0&&toPos<fromPos;
  ...
  console.info('[VYVA] Booking auto-import v10 route-names attivo');
})();
